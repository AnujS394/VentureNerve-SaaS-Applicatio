...
import {
  ...
  Clock, Download, RefreshCw, ChevronRight, Zap, BarChart2,
  Target, Calendar, FileText, ArrowUpRight, ArrowDownRight,
  Flame, Layers, CreditCard, Wallet,
} from "lucide-react";
import {
...
const cashRunwayData = [
  { month: "Jun", cash: 676000 }, { month: "Jul", cash: 640000 },
  { month: "Aug", cash: 620000 }, { month: "Sep", cash: 595000 },
  { month: "Oct", cash: 563000 }, { month: "Nov", cash: 535000 },
  { month: "Dec", cash: 510000 }, { month: "Jan '27", cash: 488000 },
  { month: "Feb '27", cash: 458000 }, { month: "Mar '27", cash: 430000 },
  { month: "Apr '27", cash: 405000 }, { month: "May '27", cash: 385000 },
...
const revenueStreams = [
  { month: "Jan", mrr: 28000, newMrr: 4200, churnedMrr: 800 },
  { month: "Feb", mrr: 31000, newMrr: 5100, churnedMrr: 600 },
  { month: "Mar", mrr: 34000, newMrr: 5400, churnedMrr: 700 },
  { month: "Apr", mrr: 36000, newMrr: 4800, churnedMrr: 1100 },
  { month: "May", mrr: 40000, newMrr: 6200, churnedMrr: 900 },
  { month: "Jun", mrr: 44000, newMrr: 6800, churnedMrr: 750 },
];

const recommendations = [
  { priority: "critical", title: "Negotiate vendor contracts Q3", impact: "Save $4,200/mo", effort: "Medium", deadline: "Jul 31, 2026", description: "Infrastructure and SaaS costs can be reduced by renegotiating annual contracts." },
  { priority: "high", title: "Accelerate MRR to $60K by Aug", impact: "+$16K revenue", effort: "High", deadline: "Aug 31, 2026", description: "Current growth trajectory needs a boost. Target 3 new enterprise accounts." },
  { priority: "high", title: "Hire Revenue Ops specialist", impact: "15% efficiency gain", effort: "Medium", deadline: "Sep 1, 2026", description: "RevOps hire will improve billing automation and reduce revenue leakage." },
  { priority: "medium", title: "Set up credit line ($200K)", impact: "Runway buffer", effort: "Low", deadline: "Jul 15, 2026", description: "Establishing a credit facility now while metrics are strong." },
];
...
const kpis = [
  ...
  { label: "Net Cash", value: "$676K", change: "-$52K", trend: "down", bad: true, icon: Wallet },
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pt-2">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => { setRefreshing(true); setTimeout(() => { setRefreshing(false); toast.success("CFO data refreshed"); }, 1400); }}
          ...
          <button onClick={() => toast.success("CFO Report exported as PDF")}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {kpis.map((kpi, i) => (
          <motion.div key={kpi.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.04 }}
            ...
            <div className={`flex items-center gap-1 mt-1 text-xs font-medium ${kpi.bad ? "text-red-500" : "text-emerald-500"}`}>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 p-6 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <div><h2 className="text-lg font-semibold text-slate-900 dark:text-white">Burn Rate vs Revenue</h2><p className="text-sm text-slate-500">Monthly comparison</p></div>
              <div className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20"><Zap className="w-3.5 h-3.5 text-emerald-500" /><span className="text-xs font-medium text-emerald-600 dark:text-emerald-400">Cash flow+ Aug '26</span></div>
            </div>
            <ResponsiveContainer width="100%" height={250}>
              <AreaChart data={burnData}>
                <defs>
                  <linearGradient id="cfo-burn" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={0.15} /><stop offset="95%" stopColor="#f43f5e" stopOpacity={0} /></linearGradient>
                  <linearGradient id="cfo-rev" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.15} /><stop offset="95%" stopColor="#10b981" stopOpacity={0} /></linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.1)" />
                <XAxis dataKey="month" tick={{ fontSize: 11 }} stroke="rgba(148,163,184,0.3)" />
                <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}K`} tick={{ fontSize: 11 }} stroke="rgba(148,163,184,0.3)" />
                <Tooltip formatter={(v: number) => [`$${v.toLocaleString()}`, ""]} contentStyle={{ background: "rgba(15,23,42,0.9)", border: "none", borderRadius: 8 }} />
                <Legend />
                <Area type="monotone" dataKey="burn" name="Burn Rate" stroke="#f43f5e" fill="url(#cfo-burn)" strokeWidth={2} dot={false} />
                <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#10b981" fill="url(#cfo-rev)" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 p-6 shadow-xl">
              ...
              <p className="text-sm text-slate-500 mb-4">June 2026 — $52,000 total</p>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart><Pie data={expenseBreakdown} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value">
                  {expenseBreakdown.map((_, i) => <Cell key={i} fill={expenseBreakdown[i].color} />)}
                </Pie><Tooltip formatter={(v: number) => [`$${v.toLocaleString()}`, ""]} contentStyle={{ background: "rgba(15,23,42,0.9)", border: "none", borderRadius: 8 }} /></PieChart>
              </ResponsiveContainer>
              <div className="space-y-2 mt-2">
                {expenseBreakdown.map((item) => (
                  <div key={item.name} className="flex items-center justify-between">
                    <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full" style={{ background: item.color }} /><span className="text-sm text-slate-600 dark:text-slate-400">{item.name}</span></div>
                  ...
                  <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100/60 dark:border-slate-800/60 last:border-0">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${tx.type === "income" ? "bg-emerald-500/10" : "bg-red-500/10"}`}>
                        {tx.type === "income" ? <ArrowUpRight className="w-4 h-4 text-emerald-500" /> : <ArrowDownRight className="w-4 h-4 text-red-400" />}
                      </div>
                      <div><p className="text-sm font-medium text-slate-800 dark:text-slate-200">{tx.desc}</p><p className="text-xs text-slate-400">{tx.date} · {tx.category}</p></div>
                    </div>
      ...
      {activeTab === "cashflow" && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 p-6 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <div><h2 className="text-lg font-semibold text-slate-900 dark:text-white">Cash Runway Projection</h2><p className="text-sm text-slate-500">No new funding scenario</p></div>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20"><AlertTriangle className="w-3.5 h-3.5 text-amber-500" /><span className="text-xs font-medium text-amber-600 dark:text-amber-400">13 months</span></div>
            </div>
            <ResponsiveContainer width="100%" height={260}>
              <AreaChart data={cashRunwayData}>
                <defs><linearGradient id="cashGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8b5cf6" stopOpacity={0.2} /><stop offset="95%" stopColor="#8b5cf6" stopOpacity={0} /></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.1)" />
                <XAxis dataKey="month" tick={{ fontSize: 11 }} stroke="rgba(148,163,184,0.3)" />
                <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}K`} tick={{ fontSize: 11 }} stroke="rgba(148,163,184,0.3)" />
                <Tooltip formatter={(v: number) => [`$${v.toLocaleString()}`, "Cash"]} contentStyle={{ background: "rgba(15,23,42,0.9)", border: "none", borderRadius: 8 }} />
                <Area type="monotone" dataKey="cash" stroke="#8b5cf6" fill="url(#cashGrad)" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Monthly Net Cash Flow</h2>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={burnData}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.1)" />
                <XAxis dataKey="month" tick={{ fontSize: 11 }} stroke="rgba(148,163,184,0.3)" />
                <YAxis tickFormatter={(v) => `$${(v / 1000).toFixed(0)}K`} tick={{ fontSize: 11 }} stroke="rgba(148,163,184,0.3)" />
                <Tooltip formatter={(v: number) => [`$${v.toLocaleString()}`, "Net"]} contentStyle={{ background: "rgba(15,23,42,0.9)", border: "none", borderRadius: 8 }} />
                <ReferenceLine y={0} stroke="rgba(148,163,184,0.4)" />
                <Bar dataKey="net" radius={[4, 4, 0, 0]}>{burnData.map((e, i) => <Cell key={i} fill={e.net >= 0 ? "#10b981" : "#f43f5e"} />)}</Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {activeTab === "revenue" && (
        <div className="space-y-6">
          <div className="rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">MRR Growth</h2>
          ...
          <div className="rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200/50 dark:border-slate-800/50 p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">New MRR vs Churned MRR</h2>
            <ResponsiveContainer width="100%" height={220}>
      ...
      {activeTab === "recommendations" && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center"><Layers className="w-4 h-4 text-white" /></div>
            <div><h2 className="text-lg font-semibold text-slate-900 dark:text-white">CFO Strategic Recommendations</h2><p className="text-sm text-slate-500">AI-generated action items ranked by financial impact</p></div>
          </div>
            ...
            <motion.div key={i} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                ...
                <div className="flex flex-row md:flex-col items-center md:items-end gap-3 flex-shrink-0">
                  <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 text-sm font-semibold"><TrendingUp className="w-4 h-4" />{rec.impact}</div>
                  <div className="flex items-center gap-1 text-slate-500 text-xs"><Calendar className="w-3.5 h-3.5" />{rec.deadline}</div>
                  <div className="flex items-center gap-1 text-slate-400 text-xs"><FileText className="w-3.5 h-3.5" />Effort: {rec.effort}</div>
                </div>
              </div>
              <div className="mt-4 pt-3 border-t border-slate-100/60 dark:border-slate-800/60 flex justify-end">
                <button onClick={() => toast.success(`"${rec.title}" added to action plan`)} className="flex items-center gap-1.5 text-sm font-medium text-violet-600 dark:text-violet-400 hover:text-violet-700 transition-colors">
                  Add to Action Plan<ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </motion.div>
          ))}
